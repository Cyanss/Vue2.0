<template>
  <div class="recommend-widget">
    <div class="widget-top" @click.stop.prevent="goShop">
      <div class="shop-pic">
    ...
    <div class="scroll-content border-bottom-1px" ref="scroll">
      <ul class="scroll-list" ref="scrollList">
        <li class="list-item" v-for="(item, index) in shopInfo.shopFoods" @click.stop.prevent="goGood">
          <span class="food-img">
            <img :src="item.image.url" width="81" height="81" >
          </span>
          <span class="food-name">{{item.foodName}}</span>
          <span class="food-price"><span class="price-sign">￥</span>{{item.foodPrice}}</span>
        </li>
      </ul>
    </div>
  </div>
  ...
  export default{
    ...
    methods: {
      ...
      goGood(event) {
        ...
        this.$router.push({path: '/shops/goods', query: { shopId: 0, goodId: 0 }});
      },
      goShop(event) {
        if (!event._constructed) {
          return;
